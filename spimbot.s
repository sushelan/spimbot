...
.text
main:
        # enable interrupts: global IE + masks
        li      $t4, 0
        ori     $t4, $t4, 1                     # global interrupt enable
...
try_solve_until_one_done:
	#Requesting one puzzle and setting current_puzzle to the last index
	li $t0, 1
	sw $t0, REQUEST_PUZZLE
	lw $t1, AVAIL_PUZZLES

    #If no puzzles go back to scanning logic
    #t1 = puzzle_count - 1
	beq  $t1, $zero, rescan
	addi $t1, $t1, -1
	sw $t1, CURRENT_PUZZLE

	#t8 = current_feedback = null
	la $t8, current_feedback_ptr
	sw $zero, 0($t8)
...
guess_loop:
	...
	#build_state(current_feedback, puzzle_state)
    #a1 = puzzle_state
	lw $a0, 0($t8)
	la $a1, puzzle_state
	jal build_state

	#guess = find_matching_word(state, words)
    #a0 = state
	la $a0, puzzle_state
	la $a1, words
	jal find_matching_word
	move $t3, $v0
	beq $t3, $zero, puzzle_failed
    ...
    #storing logic
	sw $t4, PUZZLE_FEEDBACK
	sw $t3, SUBMIT_SOLUTION

wait_feedback_i:

    #while feedback_received == 0
	la $t5, feedback_received
	lb $t6, 0($t5)
	beq $t6, $zero, wait_feedback_i

    #feedback_received = 0
	sb $zero, 0($t5)
	
	#if (is_solved(feedbacks[i])) return lgoic
	move $a0, $t4
	jal is_solved
	bne $v0, $zero, puzzle_solved

	#feedbacks[i].prev = current_feedback
	lw $t9, 0($t8)
	sw $t9, 12($t4)

	#current_feedback = feedbacks[i]
	sw $t4, 0($t8)

	#i++
	addi $t7, $t7, 1
	slti $t0, $t7, 6
	bne $t0, $zero, guess_loop

puzzle_failed:
	j try_solve_until_one_done
...
rescan:
    # HIGH-LEVEL STRATEGY:
    # 1. If our playpen was unlocked -> defend.
    # 2. If carrying many bunnies -> go home & deposit.
    # 3. If no carrots and not carrying -> solve puzzles.
    # 4. Else -> hunt nearest bunny.

    # tiny delay so interrupts breathe a bit (not a tight spinlock)
    li  $t9, 500
rescan_wait:
    addi $t9, $t9, -1
    bgtz $t9, rescan_wait

    # 1) Check if our playpen was unlocked
    la  $t0, playpen_unlocked_flag
    lb  $t1, 0($t0)
	...
	la $t0, did_lock_own_playpen
	lb $t1, 0($t0)
	beq $t1, $0, move_to_playpen_start

    # 2) Check how many bunnies we are carrying
    lw  $t2, NUM_BUNNIES_CARRIED

    # If carrying a lot (>=4), go home & deposit
    li  $t3, 4
    bge $t2, $t3, move_to_playpen_start

    # 3) If no carrots and not carrying, solve puzzles to get carrots
    lw  $t4, NUM_CARROTS
    beq $t4, $zero, rescan_need_carrots

    # 4) Otherwise we have carrots -> try to hunt bunnies
    j   rescan_scan

defend_playpen:
    # Just fall through to the movement logic
    j move_to_playpen_start

rescan_handle_defend:
    # clear flag, rush home, then lock and resume
    sb  $zero, 0($t0)
    j   defend_playpen

rescan_need_carrots:
    # If we *are* carrying bunnies but have no carrots, still bank them first
    bgtz $t2, move_to_playpen_start
    # otherwise, go solve at least one puzzle
    j   try_solve_until_one_done

rescan_scan:
    # we know NUM_CARROTS > 0 here (from rescan)
    sw $s0, SEARCH_BUNNIES

    # t1 = number of bunnies
    lw $t1, 0($s0)
    ...
    # bestDist2 = large, bestIndex = -1, i = 0
    li $t8, 0x7fffffff
    li $s1, -1
    li $t6, 0


#going through every bunny keep index of closest bunny
find_bunnies_loop:
    beq $t6, $t1, found_done

    sll  $t7, $t6, 4
    addi $t7, $t7, 4          # skip num_bunnies int
    addu $t7, $s0, $t7        # &info[i]

    lw $t2, 0($t7)            # bunny.x
    lw $t3, 4($t7)            # bunny.y

    # dist2 = (bot_x - bx)^2 + (bot_y - by)^2
    sub $t2, $t4, $t2
    mul $t2, $t2, $t2
    sub $t3, $t5, $t3
    mul $t3, $t3, $t3
...
no_bunnies_found:
    # No bunnies found – just rescan (no puzzles in Lab 14 mode)
    li  $v0, PRINT_CHAR
    li  $a0, 'P'
    syscall
...
find_target_bunny_coords:
    ...
    #t6 = address of bunny struct
    sll $t6, $s1, 4
    addi $t6, $t6, 4
    addu $t6, $s0, $t6
    
    #s2 = bunny.x, #s3 = bunny.y
    lw $s2, 0($t6)
    lw $s3, 4($t6)
...
move_x_to_bunny:
	...
	lb $t1, 0($t0)
    #if has_bonked == 0, keep continuing in x
...
wait_pickup:
    # Wait until NUM_BUNNIES_CARRIED increases above the value we had
    # in $s7 when we started the catch. Once it increases, decide whether
    # to keep hunting or go deposit based on how many we now carry.

    lw  $t2, NUM_BUNNIES_CARRIED
    ble $t2, $s7, wait_pickup_wait   # still waiting for pickup to complete

    # We successfully picked up a bunny (t2 > s7)

    # DEBUG: mark that a bunny was picked up after catch
    li  $v0, PRINT_CHAR
    li  $a0, 'b'
    syscall

    # If we are carrying at least 3 bunnies, go deposit them.
    # (Lower than the hard carry limit to stay safe.)
    li  $t5, 3
    bge $t2, $t5, move_to_playpen_start

    # Otherwise, go back to the main logic to hunt more bunnies.
    j   rescan

wait_pickup_wait:
    # Timeout loop in case pickup never happens
    addi $t3, $t3, 1
    li  $t4, 200000
    blt $t3, $t4, wait_pickup
    j   rescan


move_to_playpen_start:
    #loading where playpen is
...
move_y_to_playpen:
    ...
    #got bonked -> clear flag, stop, and rescan
    sb $zero, 0($t0)
    sw $zero, VELOCITY
    j rescan

myp_continue:
    #verifying current y position
    lw $t5, BOT_Y
    sub $t7, $s5, $t5
    beq $t7, $zero, deposit_bunnies

	slt $t0, $zero, $t7        # (0 < diff)?
...
try_lock_own_playpen:
    # Stop first to ensure MMIO success
    sw  $zero, VELOCITY
    ...
    # Success! Mark that we have qualified
    la  $t0, did_lock_own_playpen
    li  $t1, 1
    sb  $t1, 0($t0)

    # DEBUG: Print 'L' for Lock Success
    li  $v0, PRINT_CHAR
    li  $a0, 'L'
    syscall
...
tlopfail:
    # DEBUG: Print 'l' (lowercase) for lock fail
    li  $v0, PRINT_CHAR
    li  $a0, 'l'
    syscall
    jr  $ra
...
deposit_bunnies:
    ...
    # DEBUG: depositing bunnies
    li  $v0, PRINT_CHAR
    li  $a0, 'd'
    syscall

    # start deposit when carrying bunnies
    sw $t2, PUT_BUNNIES_IN_PLAYPEN

wait_drop:

	lw $t2, NUM_BUNNIES_CARRIED
	bne $t2, $zero, wait_drop

    # DEBUG: deposit finished
    li  $v0, PRINT_CHAR
    li  $a0, '0'       # "zero carried"
    syscall

dep_no_bunnies:
    li  $v0, PRINT_CHAR
    li  $a0, 'D'          # D = dep_no_bunnies
    syscall
    # we're at our playpen but carrying nothing – just try locking once

depfinish:
    # we're at our playpen now; try to lock at least once
    jal try_lock_own_playpen

    # If we have never unlocked the enemy playpen, go sabotage ONCE
    la  $t0, did_unlock_enemy_playpen
    lb  $t1, 0($t0)
    beq $t1, $zero, dep_go_sabotage_once

    # Otherwise, stop wasting time: go back to hunting bunnies
    j   rescan

dep_go_sabotage_once:
    j   go_sabotage_enemy_playpen


# SABATOGING ENEMY PLAYPEN LOGIC (SIMILAIR TO GOING TO PLAYPEN LOGIC)

attackcheck:
    # Load the flag set by the interrupt handler
    la  $t0, playpen_unlocked_flag
    lb  $t1, 0($t0)
    
    # If flag == 0 (safe), return and keep driving
    beq $t1, $zero, attackcheck_safe

    # If flag == 1 (SABOTAGE!), abort immediately
    # We do NOT clear the flag here; rescan handles that.
    
    # Stop the bot
    sw  $zero, VELOCITY
    
    # Jump to main loop to handle defense
    j   rescan
...
go_sabotage_enemy_playpen:
# DEBUG: starting sabotage path
    li  $v0, PRINT_CHAR
    li  $a0, 'E'           # Enemy
    syscall
    # load enemy playpen coordinates
    ...
    j move_x_to_enemy_playpen


move_x_to_enemy_playpen:
	jal attackcheck
    # we bonked, clear flag, stop, and rescan
    la $t0, has_bonked
    lb $t1, 0($t0)
    beq $t1, $zero, mxe_continue
...
mxe_continue:
    ...
    # diff = enemy_x - bot_x
    sub $t7, $s6, $t4
    beq $t7, $zero, move_y_to_enemy_playpen

    #  diff > 0, move east; else west
    slt $t0, $zero, $t7  
    bne $t0, $zero, face_east_enemy_pen
...
move_y_to_enemy_playpen:
	jal attackcheck
    #  we bonked, clear flag, stop, and rescan
    la $t0, has_bonked
    lb $t1, 0($t0)
    beq $t1, $zero, mye_continue
...
mye_continue:
    ...
    # diff = enemy_y - bot_y
    sub $t7, $s7, $t5
    beq $t7, $zero, enemy_unlock_playpen

    # diff > 0, move south (90°); else north (270°)
    slt $t0, $zero, $t7        # (0 < diff)?
    bne $t0, $zero, face_south_enemy_pen
    ...
    li $t3, 10
    sw $t3, VELOCITY
    j move_y_to_enemy_playpen

face_south_enemy_pen:
    sw $zero, VELOCITY
    li $t3, 90
    sw $t3, ANGLE
    li $t3, 1
...
enemy_unlock_playpen:
    # attempt to unlock enemy playpen
    li $t0, 1
    sw $t0, UNLOCK_PLAYPEN

    # check MMIO_STATUS
    lw $t1, MMIO_STATUS
    bne $t1, $zero, enemy_unlock_fail

    # success -> remember we've unlocked at least once
...
carry_limit_interrupt:
    ...
    # stop moving if we exceeded carry limit
    li $t0, 0
    sw $t0, VELOCITY

    # treat like a bonk so main code can react
    la $t0, has_bonked
    li $t1, 1
    sb $t1, 0($t0)
